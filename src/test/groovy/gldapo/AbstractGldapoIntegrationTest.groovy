...
abstract public class AbstractGldapoIntegrationTest extends AbstractServerTest
{
    ...
    void assertEqualsLdif(dn, ldif) {
        ...
        def cleanedLdif = new StringBuffer("# This LDIF file was generated by the LDIF APIs. of Novell's Java LDAP SDK\nversion: 1\ndn: $dn,$partitionSuffix\n")

        ldif.eachLine { line ->
            line = line.trim()
            if (line.size() > 0) cleanedLdif << line + "\n"
        }
        
        assertEquals(
            "$dn did not match expected ldif", 
            cleanedLdif.toString().readLines().sort().join("\n"), 
            output.toString().readLines().sort().join("\n")
        )
    }
